...
const Index = () => {
    ...
    useEffect(() => {
        ...
        utms.forEach((key) => {
            ...
            if (value) {
                ...
                // Сохраняем в сессию, чтобы метки выжили при обновлении страницы
                sessionStorage.setItem(key, value)
            } else {
                // Если в URL нет, пробуем достать из сессии
                const savedValue = sessionStorage.getItem(key)
                if (savedValue) currentUtms[key] = savedValue
            }
        })

        utmParams.current = currentUtms
    }, [])
    ...
    return (
        <div className="min-h-screen bg-background text-foreground selection:bg-primary/20 overflow-x-hidden">
            {/* Nav */}
            <nav
                className="fixed top-0 left-0 right-0 z-[100] border-b border-border/50 bg-background/80 backdrop-blur-xl">
            ...
            <section id="hero" className="relative flex min-h-[85vh] items-center pt-24 pb-12">
                <div className="container mx-auto px-6 text-center">
                    <div className="mx-auto max-w-3xl">
                        <div
                            className="mb-6 inline-flex items-center gap-2 rounded-full border border-border bg-secondary/50 px-4 py-1.5 text-[13px] sm:text-sm text-muted-foreground animate-fade-in">
                ...
                jak napisany przez człowieka
                        ...
                        <div className="flex flex-col items-center justify-center gap-4 sm:flex-row">
                            ...
                            <Button size="lg" variant="outline" onClick={() => scrollTo("how")}
                                    className="w-full sm:w-auto h-13 rounded-full px-8 text-base">
          ...
          rounded-full bg-primary/10 blur-[80px] md:blur-[120px]"
                        ...
                        {[
                            {
                                icon: FileText,
                                title: "Wklej tekst",
                                desc: "Skopiuj tekst wygenerowany przez AI i wklej go do naszego edytora."
                            },
                            {
                                icon: Sparkles,
                                title: "AI humanizuje",
                                desc: "Nasz algorytm przekształca tekst, nadając mu naturalny, ludzki styl."
                            },
                            {
                                icon: CheckCircle,
                                title: "Gotowe!",
                                desc: "Otrzymujesz tekst, który brzmi naturalnie i omija detektory AI."
                            },
                        ].map((step, i) => (
                            <div key={i}
                                 className="group relative rounded-2xl border border-border bg-card p-8 text-center transition-all hover:border-primary/30">
                                <div
                                    className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-xl bg-primary/10 text-primary transition-colors group-hover:bg-primary group-hover:text-primary-foreground">
                                    <step.icon className="h-6 w-6"/>
                                </div>
                                <div
                                    className="mb-1 text-xs font-medium text-muted-foreground uppercase tracking-wider">Krok {i + 1}</div>
                        ...
                        {[
                            {
                                ...
                                icon: Zap,
                                features: ["10 000 słów / mc", "Podstawowa humanizacja", "1 styl pisania", "Wsparcie email"],
                                popular: false
                            },
                            {
                                name: "Pro",
                                ...
                                icon: Shield,
                                features: ["50 000 słów / mc", "Zaawansowana humanizacja", "5 stylów pisania", "Priorytetowe wsparcie", "Dostęp do API"],
                                popular: true
                            },
                            {
                                name: "Premium",
                                ...
                                icon: Globe,
                                features: ["Bez limitu słów", "Najwyższa jakość", "Wszystkie style", "Dedykowany opiekun", "Dostęp do API"],
                                popular: false
                            },
                        ].map((plan) => (
                            <Card key={plan.name}
                                  className={`relative flex flex-col rounded-2xl transition-all duration-300 ${plan.popular ? "border-primary shadow-2xl shadow-primary/10 md:scale-105 z-10" : "border-border"}`}>
                                {plan.popular && <div
                                    className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-primary px-4 py-1 text-xs font-medium text-primary-foreground whitespace-nowrap">Najpopularniejszy</div>}
            ...
            <section id="cta" className="py-20">
                <div className="container mx-auto px-6">
                    <div
                        className="relative mx-auto max-w-4xl overflow-hidden rounded-[2.5rem] bg-primary px-6 py-12 text-center text-primary-foreground shadow-2xl sm:px-12 sm:py-20">
            ...
            <span className="text-xl font-bold tracking-tight font-[Space_Grotesk]">
              human<span className="text-primary">PL</span>
                        ...
                        <p className="text-sm text-muted-foreground italic text-center md:text-left">Powered by Advanced
                            Polish NLP</p>
                    </div>
